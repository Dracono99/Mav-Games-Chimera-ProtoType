...
FreeForm3DCamera::FreeForm3DCamera(D3DXVECTOR3 startPos) : m_position(startPos),m_yaw(0),m_pitch(0),m_roll(0)
{
	...
	/*m_up=D3DXVECTOR3(0.0f,1.0f,0.0f);
	m_look=D3DXVECTOR3(1.0f,0.0f,0.0f);
	m_right=D3DXVECTOR3(0.0f,0.0f,-1.0f);*/
}

FreeForm3DCamera::~FreeForm3DCamera(void)
{
}

void FreeForm3DCamera::CalculateViewMatrix(D3DXMATRIX *viewMatrix)
{
...
void FreeForm3DCamera::Yaw(float amount)
{
	m_yaw+=amount;
...
void FreeForm3DCamera::Pitch(float amount)
{
	m_pitch+=amount;
...
void FreeForm3DCamera::Roll(float amount)
{
	m_roll+=amount;
...
float FreeForm3DCamera::RestrictAngleTo360Range(float angle) const
{
...
void FreeForm3DCamera::CalcViewMatrixFromVec3PosAndQuaternion(D3DXVECTOR3& pos,D3DXQUATERNION& quat,D3DXMATRIX& mat)
{
	DragonXQuaternion q=quat;
	q.normalize();
	DragonXVector3 p = pos;
	DragonXVector3 u = q.Up();
	u.Normalize();
	DragonXVector3 r = q.Right();
	r.Normalize();
	DragonXVector3 f = q.Forward();
	f.Normalize();
	D3DXMatrixIdentity(&mat);
	D3DXMatrixLookAtLH(&mat,&pos,&f,&u);
}