...
MainOptionsMenu::MainOptionsMenu()
{
	...
	ResetBkgrndScale();
	m_GuiSet = new GuiSet(m_bkGrndTexCentr,(int)m_bkGrndTexCentr.y/(int)1.0f);
...
void MainOptionsMenu::MoveGuiElementsOffScreen()
{
	RECT rct;
...
std::vector<BaseGuiElement*> MainOptionsMenu::InitialiseGuiElements()
{
	std::vector<BaseGuiElement*> V;
	RECT rct;
	GetClientRect(gd3dApp->getMainWnd(),&rct);
	V.push_back(new Button(mSprite,mFont,m_buttonTex,OMB_AudioSettings,true,"AudioSettings",D3DXVECTOR3(170.0f, 32.0f, 0.0f),D3DXVECTOR3((float)rct.right+64.0f,(float)rct.bottom*0.5f,0.0f),D3DXVECTOR3(0.0f,0.0f,0.0f),[&](){
		gd3dApp->GetEngine()->GetAudioCore()->PlaySoundFile(m_ButtonClickEffect);
		m_GuiSet->GetGuiElementById(OMB_AudioSettings)->mState=clicked;
		MoveGuiElementsOffScreen();
		gd3dApp->GetEngine()->GetMessageSystem()->AddMessage(EngineMessage(1000,[&](){
			gd3dApp->GetEngine()->GetMenuSystem()->SwitchMenuState(MainAudioSettingsMenuState);
		}));
	}));
	return V;
}

void MainOptionsMenu::ResetBkgrndScale()
{
	RECT rct;
	GetClientRect(gd3dApp->getMainWnd(),&rct);
...
void MainOptionsMenu::Enter()
{
	ResetBkgrndScale();
	m_GuiSet->MoveGuiSetTo(m_bkGrndTexCentr);
	m_buttonClicked=false;
}
void MainOptionsMenu::Exit()
{
}
void MainOptionsMenu::OnLostDevice()
{
	HR(mSprite->OnLostDevice());
	HR(mFont->OnLostDevice());
}
void MainOptionsMenu::OnResetDevice()
{
...
void MainOptionsMenu::Update(float dt)
{
